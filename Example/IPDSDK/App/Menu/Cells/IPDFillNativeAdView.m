...
@interface IPDFillNativeAdView()



@end

@implementation IPDFillNativeAdView
...
//        [self addSubview:self.videoAdView];
        ...
        [self.imageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.mas_equalTo(SCREEN_W-10);
...
-(void)registerDataObject:(IPDNativeAdObject *)dataObject clickableViews:(nonnull NSArray<UIView *> *)clickableViews{
    ...
    if (dataObject.materialType == IPDAdMaterialTypeVideo) {
        self.videoAdView.hidden = NO;
        self.imageView.hidden = YES;
    }else if(dataObject.materialType == IPDAdMaterialTypeAtlas){
        self.imageView.hidden = YES;
